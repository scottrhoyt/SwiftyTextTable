...
class SwiftyTextTableTests: XCTestCase {
    ...
    func testRenderPerformance() {
        ...
        measure {
            ...
            XCTAssertEqual(lines.count, 1104)
        }
    }

    func presidentsTable(times: Int = 1) -> TextTable {
        let columns = [
            TextTableColumn(header: "Presidency"),
            TextTableColumn(header: "President"),
            TextTableColumn(header: "Wikipedia Entry"),
            TextTableColumn(header: "Took office"),
            TextTableColumn(header: "Left office"),
            TextTableColumn(header: "Party"),
            TextTableColumn(header: "Portrait"),
            TextTableColumn(header: "Thumbnail"),
            TextTableColumn(header: "Home State"),
            ]
        ...
        (1...count * times).forEach { index in
            table.addRow(values: rows[index % count])
        }
        return table
    }
